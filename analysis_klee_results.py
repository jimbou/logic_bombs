...
def main():
    ...
    for p in input_paths:
        ...
        for full_key, entry in data.items():
            ...
            # ensure both solvers exist
            if "z3" not in entry or "stp" not in entry:
                raise RuntimeError(f"ERROR: Missing solver in: {name}. Expected both z3 and stp.")

            merged[name] = entry
            seen_names.add(name)

            # accumulate statistics
            stats["z3"][entry["z3"]["status"]] += 1
            stats["stp"][entry["stp"]["status"]] += 1
    ...
    print("\n=== Summary ===")
    print("Z3 status counts:", dict(stats["z3"]))
    print("STP status counts:", dict(stats["stp"]))
