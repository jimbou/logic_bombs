...
def ATKrun(target, func_name='logic_bomb', default_stdin_len=10, maxtime=60, source=None, skip=False, folder=None):
    ...
    MAX_TIME = maxtime
    test_results = {}
    # ---- init helper script (fresh each run) ----
    bash_helper_path = 'bash_helper.sh'
    with open(bash_helper_path, 'w') as bf:
        ...
        for root, dirs, files in sorted(os.walk(src_dir)):
            for file in sorted(files):
                ...
                if prefix == 'mcore':
                    ...
                    # --- paths and names ---
                    src_dir_path = os.path.dirname(fp)
                    base_name, ext = os.path.splitext(os.path.basename(fp))
                    annotated_name = base_name + '_klee_annotated.c'
                    annotated_path = os.path.join(src_dir_path, annotated_name)

                    # --- build annotated content from the full combined source (res) ---
                    annotated = res  # res = original source + generated main

                    # Ensure KLEE header is present once at the very top
                    klee_header = '#include <klee/klee.h>'
                    if not annotated.lstrip().startswith(klee_header):
                        annotated = klee_header + '\n' + annotated

                    # Replace return markers with KLEE asserts
                    annotated = re.sub(
                        r'\breturn\s+BOMB_ENDING\s*;',
                        'klee_assert(0 && "Logic bomb triggered");',
                        annotated
                    )
                    annotated = re.sub(
                        r'\breturn\s+NORMAL_ENDING\s*;',
                        'klee_assert(1 && "Path without the bomb");',
                        annotated
                    )

                    # Write annotated source next to the original
                    with open(annotated_path, 'w') as f_annot:
                        f_annot.write(annotated)
                    print(f'[annotated] {annotated_path}')

                    # --- generate a clean compile command to run later INSIDE KLEE env ---
                    # Output .bc under a local 'klee' subdir next to the source
                    klee_out_dir = os.path.join(src_dir_path, 'klee')
                    out_bc_name = base_name + '_klee_annotated.bc'
                    out_bc_path = os.path.join(klee_out_dir, out_bc_name)

                    # Command: compile only, emit LLVM bitcode, include headers (adjust include path if needed)
                    # NOTE: we do not add linker flags (-l...) here; compile step only
                    # We also ensure the 'klee' output dir exists via 'mkdir -p' in the bash script
                    compile_cmd = (
                        f'mkdir -p {shlex.quote("klee")} && '
                        f'clang -emit-llvm -c -g '
                        f'-I include '  # adjust/include more -I as your project needs
                        f'{shlex.quote(annotated_name)} '
                        f'-o {shlex.quote(os.path.join("klee", out_bc_name))}'
                    )

                    # Append a runnable line to bash_helper.sh:
                    # cd into the source dir so relative paths (include/, klee/, file.c) resolve naturally
                    with open(bash_helper_path, 'a') as bf:
                        bf.write(f'cd {shlex.quote(src_dir_path)} && {compile_cmd}\n')

                    # Mark as a successful prepare step (no compile/run now)
                    test_results[fp] = CORRECT
                    continue
